...
class TensorNeRF(torch.nn.Module):
    ...
    def get_optparam_groups(self, lr_init_spatial=0.02, lr_init_network=0.001, lr_bg=0.03, lr_scale=1):
        grad_vars = []
    ...
    def forward(self, rays_chunk, focal, recur=0, init_refraction_index=torch.tensor(1.0), override_near=None, output_alpha=None, white_bg=True, is_train=False, ndc_ray=False, N_samples=-1):
        ...
        if app_mask.any():
            ...
            diffuse, tint, ambient, refraction_index, reflectivity, ratio_diffuse = self.diffuse_module(
            ...
            if recur >= self.max_recurs and self.ref_module is None:
                ...
                if bounce_mask.sum() > 0:
                    ...
                    tinted_ref_rgb = brdf * incoming_light * cos_lamb
            ...
            rgb[app_mask] = tint * ((1-reflectivity)*ambient + reflectivity * reflect_rgb)
