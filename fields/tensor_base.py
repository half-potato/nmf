...
class TensorBase(torch.nn.Module):
    ...
    def normalize_coord(self, xyz_sampled):
        ...
        if self.contract_space:
            ...
            d = 3
            dist = (
                torch.linalg.norm(normed[..., :d], dim=-1, keepdim=True, ord=2) + 1e-8
            )
    ...
    def compute_normals(self, xyz):
        with torch.enable_grad():
            ...
            # compute sigma
            validsigma = self.compute_densityfeature_with_xyz_grad(
                xyz_g, activate=False
            )
    ...
    def compute_densityfeature_with_xyz_grad(self, xyz_sampled, activate=True):
        if xyz_sampled.shape[0] == 0:
            return torch.empty((0), device=xyz_sampled.device)
        sigfeat = self._compute_densityfeature_with_xyz_grad(
            self.normalize_coord(xyz_sampled)
        )
        if activate:
            return self.feature2density(sigfeat).reshape(-1)
        else:
    ...
    def _compute_densityfeature_with_xyz_grad(self, xyz_sampled):
        return self._compute_densityfeature(xyz_sampled)
...
class TensorVoxelBase(TensorBase):
    def __init__(
        ...
        **kwargs,
        ...
        self.comp_w = [1, 1, 1]
        grid_size = (
            torch.tensor(utils.N_to_reso(N_voxel_init, self.aabb))
