...
# def grid_sample2(image, optical, smoothing, **kwargs):
...
class TensoRF(torch.nn.Module):
    ...
    def init_one_svd(self, n_component, grid_size, scale, shift):
        ...
        for i in range(len(self.vecMode)):
            ...
            match self.init_mode:
                ...
                case "integrated":
                    b = safemath.integrated_pos_enc(
                        (
                            pos_vals.reshape(-1, 1) * torch.pi,
                            torch.zeros_like(pos_vals).reshape(-1, 1),
                        ),
                        0,
                        n_degs,
                    )
                    ...
                    a = safemath.integrated_pos_enc(
                        (
                            line_pos_vals.reshape(-1, 1) * torch.pi,
                            torch.zeros_like(line_pos_vals).reshape(-1, 1),
                        ),
                        0,
                        n_degs,
                    )
                    ...
                    line_coef_v = a
                case "randplane":
                    plane_coef_v = scale ** (1 / 2) * torch.randn(
                        (1, n_component, grid_size, grid_size)
                    )
                    line_coef_v = scale ** (1 / 2) * torch.ones(
                        (1, n_component, grid_size, 1)
                    )
                case "unifplane_scaled":
                    scales_r = scales.repeat_interleave(2, dim=1)
                    plane_coef_v = scales_r * (
                        2 * torch.rand((1, n_component, grid_size, grid_size)) - 1
                    )
                    line_coef_v = scales_r * torch.ones((1, n_component, grid_size, 1))
                case "unifplane":
                    plane_coef_v = scale ** (1 / 2) * (
                        2 * torch.rand((1, n_component, grid_size, grid_size)) - 1
                    )
                    line_coef_v = scale ** (1 / 2) * torch.ones(
                        (1, n_component, grid_size, 1)
                ...
                case "unif":
                    ...
                    line_coef_v = scale ** (1 / 2) * (
                        2 * torch.rand((1, n_component, grid_size, 1)) - 1
                    )
                case _:
                    plane_coef_v = scale * torch.randn(
                        (1, n_component, grid_size, grid_size)
                    )
    ...
    def coords(self, xyz_normed):
        coordinate_plane = torch.stack(
            (
                xyz_normed[..., self.matMode[0]],
                xyz_normed[..., self.matMode[1]],
                xyz_normed[..., self.matMode[2]],
            )
        ).view(3, -1, 1, 2)
        coordinate_line = torch.stack(
            (
                xyz_normed[..., self.vecMode[0]],
                xyz_normed[..., self.vecMode[1]],
                xyz_normed[..., self.vecMode[2]],
            )
        )
        coordinate_line = torch.stack(
...
class TensorVMSplit(TensorVoxelBase):
    def __init__(
        ...
        **kwargs,
        ...
        self.density_rf = model(
            self.grid_size[0],
            self.density_n_comp[0],
            init_mode,
            interp_mode,
            d_init_val,
            self.lr,
        ...
        grad_vars = [
            {
                ...
                "lr": lr_scale * self.lr_net,
                "betas": [0.9, 0.99],
            },
            {
                "params": self.dbasis_mat.parameters(),
                "lr": lr_scale * self.lr_net,
                "betas": [0.9, 0.99],
            },
    ...
    def density_L1(self):
        total = 0
        for idx in range(len(self.density_rf.app_plane)):
            total = (
                total
                + torch.mean(torch.abs(self.density_rf.app_plane[idx]))
                + torch.mean(torch.abs(self.density_rf.app_line[idx]))
    ...
    def TV_loss_density(self, reg):
        ...
        for idx in range(len(self.density_rf.app_plane)):
            total = (
                total
                + reg(self.density_rf.app_plane[idx]) * 1e-2
                + reg(self.density_rf.app_line[idx]) * 1e-3
            )
        return total

    def TV_loss_app(self, reg, start_ind=0, end_ind=-1):
        total = 0
        for idx in range(len(self.app_rf.app_plane)):
            total = (
                total
                + reg(self.app_rf.app_plane[idx]) * 1e-2
                + reg(self.app_rf.app_line[idx]) * 1e-3
            )
        return total
    ...
    def _compute_densityfeature_with_xyz_grad(self, xyz_sampled):
        sigma_feature = self.density_rf(xyz_sampled, enable_xyz_grad=True)
